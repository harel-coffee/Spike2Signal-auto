...
# X = np.random.rand(7000, 1, 1274)
# y = np.random.randint(0, 3, (7000,))
...
learn.plot_metrics()
test_probas2, test_targets2, test_preds2 = learn.get_X_preds(X[splits_new[2]], with_decoded=True)
...
# test_targets3
aa = pd.DataFrame(list(test_preds2))
# aa.drop("]", axis=0)
aa.columns = ["name"]
aa = aa.loc[aa["name"] != "["]
aa = aa.loc[aa["name"] != ","]
...
final_org_vals = y[splits_new[2]]


# In[20]:


len(final_pred_vals),len(final_org_vals)
