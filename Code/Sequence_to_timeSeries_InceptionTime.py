...
import sys
sys.stdout = open('./output_inception.txt', 'w')
...
splits_new_2 = get_splits(np.array(y), valid_size=.10, test_size=.20, random_state=23, stratify=True)
...
splits_new = get_splits(np.array(y), valid_size=.10, test_size=.20, random_state=23, stratify=True)
splits_new
...
batch_tfms_test = [TSStandardize()]
dsets_testing = TSDatasets(final_test_x, final_test_y, tfms=tfms_test, splits=splits_testing)
dls_testing = TSDataLoaders.from_dsets(dsets_testing.train, dsets_testing.valid, bs=[64, 128], batch_tfms=batch_tfms_test)
...
aa, preds, targets = learn.get_preds(dl=dls_testing.valid, with_decoded=True)
# learn.get_preds(dl=dls_testing.valid, y_test_new, with_decoded=True)

# preds = test_preds2[:]
# targets = test_targets2[:]

# aa, preds, targets = learn.get_preds(dl=dls.valid, with_input=True)
# preds
